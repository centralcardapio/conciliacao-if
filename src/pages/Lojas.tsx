...
type SortField = 'id' | 'nome' | 'regional';
...
const Lojas: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ nome: '', regionalId: '' });
  ...
  const openEditModal = (loja: Loja) => {
    setEditingLoja(loja);
    setFormData({ nome: loja.nome, regionalId: loja.regionalId });
    setFormError('');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingLoja) {
      setLojas(prev =>
        prev.map(l =>
          l.id === editingLoja.id 
            ? { ...l, nome: formData.nome.trim(), regionalId: formData.regionalId } 
            : l
        )
      );
    } else {
